...
typedef void (*Integrand)(double x, int fValues_length, double* fValues);
typedef void (*IntegrandComplex)(double x, int fValues_length, std::complex<double>* fValues);

class HankelTransformsByAnderson
{
    const double e02 = 1.22140275816017; // exp(0.2)
    const double einv = 0.818730753077982; // 1/exp(0.2)

   // J0 TRANSFORM FILTER WEIGHT ARRAYS (EQUIVALENT TO WTO ARRAY)
    const double WT0[283] = {
   ...
   -2.6163525E-08, 3.6808773E-07, 2.7049871E-08, 4.7932617E-07,
    ...
    //  J1 TRANSFORM FILTER WEIGHT ARRAYS (EQUIVALENT TO WT1 ARRAY)

    const double WT1[283] = {
   ...
   -9.5104370E-16, 1.4600474E-16, 5.6166519E-16, 8.2899246E-17,
    ...
    // Параметры метода
    // Integrand f - указатель на функцию с прототипом 
    // typedef void (*Integrand)(double x, int fValues_length, double* fValues);
    //    x - значение аргумента, для которого вычисляется векторная подинтегральная функция
    //    fValues_length - число элементов в векторной подинтегральной функции
    //    для первых j0_n элементов при вычислении интегралов будет вычислено преобразование Ханкеля с J0
    //    для остальных элементов при вычислении интегралов будет вычислено преобразование Ханкеля с J1
    //    j0_n <= fValues_length; как параметр не передается
    // 
    // double r - расстояние
    // double eps - относительная точность
    // int j0_n - число интегралов, для которых вычисляется преобразование Ханкеля с J0
    // int j1_n - число интегралов, для которых вычисляется преобразование Ханкеля с J1
    // double* res - массив длиной j0_n + j1_n с результатами
    // double* c - не используется в данной реализации, но память надо распределять  
    // double* cmax - не используется не используется в данной реализации, но память надо распределять  
    //
    // 
    // Всегда используется длинный фильтр. eps и массив cmax не используются
    public:
     void call(Integrand f, double r, double eps, int j0_n, int j1_n,
               double* res, double* c, double* cmax)
    {

        int js = j0_n + j1_n;
        bool none = true;

        //-----INITIALIZE KERNEL ABSCISSA GENERATION FOR GIVEN R
        double y1 = 0.7358852661479794460 / r;
        for (int j = 0; j < js; j++)
        {
            res[j] = 0;
            cmax[j] = 0;
        ...
        {
            //Console.WriteLine("I= {0} WT0[I]= {1} WT1[I]= {1}", I, WT0[I], WT1[I]);
            y = y * e02;
            f(y, js, c);
            for (int j = 0; j < j0_n; j++)
            {
                c[j] = c[j] * WT0[I];
                res[j] = res[j] + c[j];
            }
            for (int j = j0_n; j < js; j++)
            {
                c[j] = c[j] * WT1[I];
                res[j] = res[j] + c[j];
            }
            ...
            //for (int j = 0; j < js; j++)
            //{ if (cmax[j] < Math.Abs(c[j])) cmax[j] = Math.Abs(c[j]); }  
        }

        //double cmin = cmax[0] ;
        //for (int j=1; j<js; j++)
        //   { if ( cmin > cmax[j]) cmin = cmax[j]; }
        //if (cmin <= Double.Epsilon) none = false;

        //for (int j=0; j<js; j++) cmax[j] = cmax[j]*eps;        


  //-----CHECK FOR FILTER TRUNCATION AT RIGHT END


        for (int I = 149; I < 283; I++)
        {
            //Console.WriteLine("I= {0} WT0[I]= {1} WT1[I]= {1}", I, WT0[I], WT1[I]);
            //bool yes = true;
            //for (int j=0; j<js; j++) if ( Math.Abs(c[j]) > cmax[j]) yes = false;
            //if ( yes ) break;
        ...
        for (int I = 129; I >= 0; I--)
        {
            //Console.WriteLine("I= {0} WT0[I]= {1} WT1[I]= {2}", I, WT0[I], WT1[I]);
            //bool yes = true;
            //for (int j=0; j<js; j++) if ( Math.Abs(c[j]) > cmax[j]) yes = false;
            //if ( yes && none ) break;
        ...
        for (int j = 0; j < js; j++) res[j] = res[j] / r;
    }

     // Проверяется относительная погрешность eps и может использоваться коротокий фильтр. 
    public:
        void call_1(Integrand f, double r, double eps, int j0_n, int j1_n,
            double* res, double* c, double* cmax)
        {
 ...
 // int j0_n - число интегралов, для которых вычисляется преобразование Ханкеля с J0
 // int j1_n - число интегралов, для которых вычисляется преобразование Ханкеля с J1
 // complex<double>* res - массив длиной j0_n + j1_n с результатами
 // complex<double>* c - не используется в данной реализации, но память надо распределять 
 // complex<double>* cmax - не используется не используется в данной реализации, но память надо распределять  
 //
 // 
 // Всегда используется длинный фильтр. eps и массив cmax не используются
    public:
        void callComplex(IntegrandComplex f, double r, double eps, int j0_n, int j1_n,
            complex<double>* res, complex<double>* c, complex<double>* cmax)
        {
